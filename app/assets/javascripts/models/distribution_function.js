function DistributionFunction(algorithms) {
    ...
    this.pdf_states = ['diagram', 'nuclear_pdf', 'histogram']
...
distributionFunctionProto.plotCdf = function(div_id) {
    ...
    for(var algorithm_name in this.algorithms) {
        ...
        this.algorithms[algorithm_name]['cdf'].push([500, 1])
        data.push(
            {
                data: this.algorithms[algorithm_name]['cdf'],
...
distributionFunctionProto.plotHistogram = function(div_id) {
    ...
    for(var algorithm_name in this.algorithms) {
        ...
        data.push(
            {
                data: this.algorithms[algorithm_name]['histogram'],
                label: algorithm_name,
                color: this.lines_options[line_id].color,
                points: this.points_data_options(line_id),
                bars: {
...
distributionFunctionProto.plotDiagram = function(div_id) {
    ...
    for(var algorithm_name in this.algorithms) {
        ...
        var errors_parameters = this.algorithms[algorithm_name]['errors_parameters']
        ...
        canvas.drawLine(mean, y_top, mean, y_bottom, 0, 1, 'brown')
        canvas.drawLine(median, y_top, median, y_bottom, 0, 3)
...
distributionFunctionProto.plotNuclearPdf = function(div_id) {
    var self = this
    var max = 0.0

    function nuclear_pdf_value(x, sigma) {
        var sum = 0.0
        var errors = self.algorithms[algorithm_name]['errors']
        for(var i in errors) {
    ...
    for(var algorithm_name in this.algorithms) {
        ...
        var sigma = this.algorithms[algorithm_name]['errors_parameters']['stddev']

        for(var x = 0; x < this.pdf_options.xaxis.max; x += 1) {
            var y = nuclear_pdf_value(x, sigma)
            if(y > max)max = y
